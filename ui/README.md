...
- `loading_overlay.gd` is the read-only transition shown after selecting a
  world. It reports secure spawn-ring and full-horizon stream counts from
  `VoxelWorld`, consumes input until local collision commits, then fades without
  delaying gameplay while the HUD continues to report background streaming.
