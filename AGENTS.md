...
- Controls: WASD, Space jump, double-tap Space toggles fly, Shift sprint, Ctrl fast fly (`fly_boost`), E inventory (`inventory`), 1-9/0 and mouse wheel select hotbar slot, LMB mine, RMB place, F1 HUD (`hud_toggle`), F2 screenshot (`screenshot`), P photo camera (`photo_camera`), Esc pause (`ui_cancel`). In the photo camera WASD flies, Space/Shift rise/descend, Ctrl boosts, the wheel zooms, and Esc exits.

## UI design system

- `ui/ui_theme.gd` is the shared Deepslate & Ember theme and component factory; keep palette, fonts, style boxes, control textures, and button variants centralized there instead of styling widgets independently.
