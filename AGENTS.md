...
- Footsteps are distance- and material-aware (`player/player.gd`); block break/place use one uniform subtle cue for all blocks. UI clicks fire from `UITheme` button styles, and rain ambience follows `WeatherSystem`. Master/SFX/Ambience sliders live in the settings panel and persist through `GameConfig`.
