...
- Water: non-opaque, non-breakable, no collision; meshed as its own surface and gets the same `ARRAY_CUSTOM0` light attribute as blocks (`_append_water_face()` samples the volume, so `water.gdshader` can darken by sky light and add block-light emission). `VoxelWorld.is_water_at()` drives the HUD underwater overlay.
